...
class _EnquiryDetailScreenState extends State<EnquiryDetailScreen> {
  ...
  Future<void> _shareEnquiry(BuildContext context) async {
    final e = widget.enquiry;
    final dateStr = DateFormat('dd-MMM-yyyy').format(e.createdAt);
    ...
    if (e.requirements != null && e.requirements!.isNotEmpty) {
      buffer.writeln('*Requirements*:');
      buffer.writeln(e.requirements);
      buffer.writeln('');
    }

    if (e.expectedWindows != null && e.expectedWindows!.isNotEmpty) {
      buffer.writeln('*Expected Windows*:');
      buffer.writeln(e.expectedWindows);
      buffer.writeln('');
    }

    if (e.notes != null && e.notes!.isNotEmpty) {
      buffer.writeln('*Notes*:');
      buffer.writeln(e.notes);
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            icon: const AppIcon(AppIconType.share),
            onPressed: () => _shareEnquiry(context),
          ),
          IconButton(
            icon: const AppIcon(AppIconType.edit),
            onPressed: () => _edit(context),
          ),
          IconButton(
            icon: const AppIcon(AppIconType.delete, color: Colors.red),
            onPressed: _delete,
          ),
        ],
      ),
