...
class _WindowInputScreenState extends State<WindowInputScreen> {
  ...
  double get _totalSqFt {
    ...
    for (var c in _windowControllers) {
      if (!c.isOnHold || settings.countHoldOnTotal) {
        ...
        Row(
          children: [
            ...
            GestureDetector(
              ...
              child: AppIcon(
                isOnHold ? AppIconType.play : AppIconType.pause,
                color: isOnHold
                    ? Colors.green
...
class WindowInputController {
  ...
  factory WindowInputController.fromWindow(Window w) {
    String fmt(double v) =>
        v.remainder(1) == 0 ? v.toInt().toString() : v.toString();
    return WindowInputController(
      id: w.id,
      name: w.name,
      initialWidth: fmt(w.width),
      initialHeight: fmt(w.height),
      initialWidth2: w.width2 != null ? fmt(w.width2!) : null,
